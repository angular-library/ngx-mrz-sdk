// Refer to https://github.com/Dynamsoft/label-recognizer-javascript-demo/blob/master/src/mrz-parser.ts
import { Component, OnInit } from '@angular/core';

@Component({
  selector: 'lib-ngx-mrz-parser',
  templateUrl: './ngx-mrz-parser.component.html',
  styleUrls: ['./ngx-mrz-parser.component.css']
})

export class NgxMrzParserComponent implements OnInit {
  ...
  static parseTwoLines(line1: string, line2: string) {
    ...
    parseResultInfo.value.splice(0, parseResultInfo.value.length);
    for(let i in passportMRZ) {
        parseResultInfo.value.push(passportMRZ[i]);
    }
    return parseResultInfo.value;
  };
  
  static parseThreeLines(line1: string, line2: string, line3: string) {
    const parseResultInfo: any = [];
    let passportMRZ : any = {};
    let type = line1.substring(0, 1);
    if(!(/[I|P|V]/.test(type))) return false;
